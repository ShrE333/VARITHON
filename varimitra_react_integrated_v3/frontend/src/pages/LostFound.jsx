...
import { Camera, Check, Clock3, RefreshCw, Upload, UserRoundSearch, X, MapPin, Activity, CloudDownload } from 'lucide-react';
import AdminShell from '../components/AdminShell';
import StatusPill from '../components/StatusPill';
import { LOST_API, getJson, postJson } from '../api';

export default function LostFound(){
  const [health,setHealth]=useState(null),[cases,setCases]=useState([]),[cameras,setCameras]=useState([]),[alerts,setAlerts]=useState([]),[sightings,setSightings]=useState([]),[busy,setBusy]=useState(false),[msg,setMsg]=useState(''),[reportId,setReportId]=useState('');
  ...
  const ingestReport=async(e)=>{
    ...
    setBusy(true);setMsg('');
    try{
      const r=await fetch(`${LOST_API}/reports/${encodeURIComponent(id)}/ingest`,{method:'POST'});
      const body=await r.json().catch(()=>({detail:'Unknown server response'}));
      if(!r.ok)throw new Error(body.detail||JSON.stringify(body));
      if(body.report_type==='lost')setMsg(`Lost report ${id} ingested from GCS. CCTV monitoring is active; no camera restart required.`);
      else setMsg(`Found report ${id} ingested. ${body.candidate_lost_matches?.length||0} active lost-case candidate(s) created.`);
      setReportId('');
      await load();
    }catch(err){setMsg(`Could not ingest report: ${err.message}`)}finally{setBusy(false)}
  };
  ...
  return <AdminShell title="Lost & Found AI" subtitle="Single-photo face recognition · local InsightFace mode">
    ...
    <div className="two-col">
      <section className="panel">
        <div className="panel-title"><h2><CloudDownload/> GCS Report Intake</h2><span>Primary automated flow</span></div>
        <form className="case-form" onSubmit={ingestReport}>
          <label>Lost / Found Report ID
            <input value={reportId} onChange={e=>setReportId(e.target.value)} placeholder="LF-20260829-D69ED1AF" required/>
          </label>
          <p className="form-hint">The backend searches both <code>lost/&lt;ID&gt;/original.jpg</code> and <code>found/&lt;ID&gt;/original.jpg</code> in the configured Google Cloud Storage bucket.</p>
          <button className="primary-btn" disabled={busy}>{busy?'Fetching from GCS…':'Fetch & Start Automation'}</button>
          {msg&&<p className="form-message">{msg}</p>}
        </form>
      </section>
      <section className="panel"><div className="panel-title"><h2><Activity/> Automated behavior</h2></div><div className="explain-list"><div><b>Lost report</b><span>Image is downloaded, enrolled, and becomes an ACTIVE CCTV case. Running cameras discover it automatically.</span></div><div><b>Found report</b><span>Image is downloaded and compared immediately against every active lost-person case.</span></div><div><b>Cameras</b><span>Workers remain running even when there are zero cases and refresh the active registry continuously.</span></div></div></section>
    </div>
