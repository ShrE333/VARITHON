...
def init_db():
    ...
    with connect() as c:
        ...
        c.execute("""CREATE TABLE IF NOT EXISTS camera_status(
            ...
            faces INTEGER DEFAULT 0,last_seen TEXT,error TEXT)""")
...
def update_camera_status(camera_id,camera_location,source,online,fps=0.0,faces=0,error=None):
    now=datetime.now(timezone.utc).isoformat()
    with connect() as c:
        c.execute("""INSERT INTO camera_status(camera_id,camera_location,source,online,fps,faces,last_seen,error)
                     VALUES(?,?,?,?,?,?,?,?)
                     ...
                     last_seen=excluded.last_seen,error=excluded.error""",
                  (camera_id,camera_location,str(source),1 if online else 0,float(fps),int(faces),now,error))
