...
class VideoScanner:
    ...
    def scan(self,source,camera_id,camera_location,display=True):
        ...
        fpsq=deque(maxlen=FPS_SMOOTHING); idx=0; fps=0.0; face_count=0; last_hb=0.0; last_refresh=0.0
        update_camera_status(camera_id,camera_location,source,True,0,0,None)
        try:
            while True:
                ...
                if idx%PROCESS_EVERY_N_FRAMES==0 and cases:
                    small,s=self.resize(frame); inv=1/s; faces=self.face_engine.detect(small); face_count=len(faces)
                    boxes=[tuple(int(v) for v in f.bbox) for f in faces]; tids=self.tracker.update(boxes); anns=[]
                    for face,b,tid in zip(faces,boxes,tids):
                        ...
                        if m:
                            ...
                            anns.append({'bbox':(x1,y1,x2,y2),'label':f'T{tid}','matched':False})
                    self.cached=anns
                self.draw(frame,self.cached); dt=time.perf_counter()-t0
                if dt>0: fpsq.append(1/dt); fps=sum(fpsq)/len(fpsq)
                if now-last_hb>1.5:
                    update_camera_status(camera_id,camera_location,source,True,fps,face_count,None); last_hb=now
                cv2.putText(frame,f'{camera_id} | FPS: {fps:.1f} | Faces: {face_count}',(18,34),cv2.FONT_HERSHEY_SIMPLEX,.72,(0,255,0),2)
                if display:
                    cv2.imshow(f'VariMitra V4 - {camera_id}',frame)
                    if cv2.waitKey(1)&0xff==ord('q'): break
        except Exception as e:
            update_camera_status(camera_id,camera_location,source,False,fps,face_count,str(e)); raise
        finally:
            cap.release()
            if display: cv2.destroyAllWindows()
            update_camera_status(camera_id,camera_location,source,False,fps,face_count,'Stream stopped')
