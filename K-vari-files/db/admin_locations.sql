...
-- ------------------------------------------------------------
-- 5. New category: 'temple' — added for the Super Admin dashboard's
--    prominent SOS/Locations entry point, so admins can also mark temple
--    locations on the same map used for medical/SOS points.
--
--    Run this once, manually, in the Supabase SQL editor. It is idempotent.
--    Until it is run, records with category 'temple' still write and read
--    fine (the API only validates against lib/admin-locations/categories.ts,
--    not this enum) — they just fall back to a generic pin wherever code
--    reads the enum's contents directly, per categoryOrFallback().
-- ------------------------------------------------------------
