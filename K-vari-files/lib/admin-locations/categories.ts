 ...
 *
 * TO ADD A CATEGORY:
 *   1. Add an entry to LOCATION_CATEGORIES below.
 *   2. Add the matching value to the database enum:
 *        alter type facility_kind add value if not exists 'your_id';
 *      (see db/admin_locations.sql)
 *
 * TO RETIRE A CATEGORY: remove it from this array only. Postgres cannot
 * drop an enum value, but nothing here reads the enum's contents — existing
 * rows keep working, and the category simply stops being offered.
 *
 * `id` values match the database's `facility_kind` enum exactly, which is
 * what lets admin-created records appear in the existing pilgrim-facing SOS
 * feature without a translation layer.
 */
...
export interface CategoryDefinition {
  ...
  color: string;
  /**
   * Broad grouping the pilgrim-facing UI already uses (lib/types.ts
   * KIND_GROUPS). Categories with no natural group use 'other'.
   */
...
export const LOCATION_CATEGORIES: CategoryDefinition[] = [
  ...
  { id: 'temple', label: 'Temple', icon: '🛕', color: '#ea580c', group: 'other' },
