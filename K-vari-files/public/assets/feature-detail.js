(function(){
  ...
  function render(){
    ...
    if (ctaText && ctaBtn) {
      if (key === 'eticketing' && userRole !== 'admin') {
        ctaText.textContent = 'Book your Vari darshan slot online with instant QR ticket confirmation.';
        ctaBtn.href = '/darshan-booking?back=' + encodeURIComponent(back);
        ctaBtn.textContent = 'Book Darshan Now';
      } else if (userRole === 'admin') {
        ctaText.textContent = t('pilgrim.manageCta').replace('{title}', data.title);
        ctaBtn.href = "/command-dashboard";
    ...
    if (relWrap) {
      ...
      relWrap.innerHTML = picked.map(function(k){
        var f = getFeature(k);
        return '<a class="fd-rcard" href="/feature?key=' + encodeURIComponent(k) + '&back=' + encodeURIComponent(back) + '">' +
