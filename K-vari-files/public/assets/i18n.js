...
(function(){
  ...
  function initLanguagePickers(){
    ...
    /* boot() runs again on every client-side navigation, so this outside-click
       handler is registered once rather than once per page view. */
  ...
  window.VariMitraI18n = {
    ...
    apply: apply,
    /* Exposed for the merged app: client-side routing swaps the page's
       markup without a reload, so the router has to re-run setup against
       the new DOM. Idempotent -- initLanguagePickers skips anything it has
       already wired via the data-i18n-ready flag. */
    boot: function(){ boot(); }
  };
