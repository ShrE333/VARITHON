 ...
 * imported here. The implementation is code-split with React.lazy, which
 * is required rather than stylistic: Leaflet touches `window` at import
 * time and will crash any server render (Next RSC, Remix, SSR Vite).
 *
...
import type { AdminLocation, LocationCategory } from '../types';
...
const Impl = lazy(() => import('./providers/LeafletLocationMap'));
...
export function LocationMap(props: LocationMapProps) {
  return (
    <Suspense fallback={<MapSkeleton height={props.height ?? 400} />}>
      <Impl {...props} />
    </Suspense>
  );
}
