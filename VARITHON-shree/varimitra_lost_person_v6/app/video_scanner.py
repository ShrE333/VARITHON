...
class VideoScanner:
    ...
    def publish_live_frame(self,camera_id,frame):
        # Unique filenames avoid Windows WinError 5 while the browser is reading a JPEG.
        LIVE_DIR.mkdir(parents=True,exist_ok=True)
        safe=''.join(ch if ch.isalnum() or ch in '-_' else '_' for ch in camera_id)
        self.live_seq+=1
        p=LIVE_DIR/f'{safe}_{int(time.time()*1000)}_{self.live_seq}.jpg'
        ok,buf=cv2.imencode('.jpg',frame,[int(cv2.IMWRITE_JPEG_QUALITY),78])
        if not ok:return
        try:p.write_bytes(buf.tobytes())
        except OSError:return
        files=sorted(LIVE_DIR.glob(f'{safe}_*.jpg'),key=lambda x:x.stat().st_mtime,reverse=True)
        for old in files[20:]:
            try:old.unlink()
            except OSError:pass
    def scan(self,source,camera_id,camera_location,display=True):
        ...
        if not cap.isOpened():
            update_camera_status(camera_id,camera_location,source,False,error='Could not open source');raise RuntimeError(f'Could not open source: {source}')
        try:cap.set(cv2.CAP_PROP_BUFFERSIZE,1)
        except:pass
        fpsq=deque(maxlen=FPS_SMOOTHING);idx=0;fps=0.;face_count=0;matched_count=0;last_hb=0.;last_refresh=0.;last_live=0.
        ...
        try:
            while True:
                ...
                if now-last_refresh>3.0:cases=self.registry.list_active();last_refresh=now
                if idx%PROCESS_EVERY_N_FRAMES==0:
                    small,s=self.resize(frame);inv=1/s
                    faces=self.face_engine.detect_and_embed(small);face_count=len(faces);matched_count=0
                    ...
                    for face,b,tid in zip(faces,boxes,tids):
                        ...
                        emb=self.face_engine.normalized_embedding(face);m=self.matcher.best_match(emb,cases) if cases else None
                        if m:
                            matched_count+=1;anns.append({'bbox':(x1,y1,x2,y2),'label':f'T{tid} MATCH {m.name} {m.similarity:.3f}','matched':True})
                            if self.matcher.register(m,tid):
                                ts=datetime.now(timezone.utc);p=ALERTS_DIR/f"{m.case_id}_{camera_id}_{ts.strftime('%Y%m%dT%H%M%S%fZ')}.jpg"
                                ...
                                a['alert_id']=insert_alert(a);print('\nADMIN ALERT');print(json.dumps(a,indent=2))
                        else:anns.append({'bbox':(x1,y1,x2,y2),'label':f'T{tid} FACE','matched':False})
                    self.cached=anns
                self.draw(frame,self.cached)
                dt=time.perf_counter()-t0
                if dt>0:fpsq.append(1/dt);fps=sum(fpsq)/len(fpsq)
                if now-last_hb>1.0:update_camera_status(camera_id,camera_location,source,True,fps,face_count,matched_count,None);last_hb=now
                ...
                if now-last_live>=0.12:self.publish_live_frame(camera_id,frame);last_live=now
                if display:
                    cv2.imshow(f'VariMitra V6 - {camera_id}',frame)
