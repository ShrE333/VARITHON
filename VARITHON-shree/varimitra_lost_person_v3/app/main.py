...
app=FastAPI(title='VariMitra Lost Person API',version='0.3.0'); init_db(); engine=FaceEngine(False); registry=CaseRegistry()
@app.get('/health')
...
@app.get('/alerts')
def alerts(status:str|None=None): return list_alerts(status)
@app.get('/alerts/{alert_id}/evidence')
def evidence(alert_id:int):
    r=next((x for x in list_alerts() if x['alert_id']==alert_id),None)
...
def confirm(alert_id:int): return review_alert(alert_id,'CONFIRMED')
@app.post('/alerts/{alert_id}/reject')
def reject(alert_id:int): return review_alert(alert_id,'REJECTED')
