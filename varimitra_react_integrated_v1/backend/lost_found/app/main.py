...
app=FastAPI(title='VariMitra Lost Person API',version='0.5.1-react')
app.add_middleware(CORSMiddleware, allow_origins=['*'], allow_credentials=False, allow_methods=['*'], allow_headers=['*'])
init_db(); engine=FaceEngine(False); registry=CaseRegistry()
...
async def camera_stream(camera_id:str):
    ...
    async def gen():
        ...
        while True:
            try:
                frames=sorted(LIVE_DIR.glob(f'{safe}_*.jpg'),key=lambda x:x.stat().st_mtime,reverse=True)
                p=frames[0] if frames else None
                data=p.read_bytes() if p and p.exists() else None
