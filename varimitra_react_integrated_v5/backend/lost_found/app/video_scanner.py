...
class VideoScanner:
    ...
    def publish_live_frame(camera_id, frame):
        # Never overwrite the JPEG currently being read by the browser on Windows.
    ...
    @staticmethod
    def _is_file_source(source):
        return isinstance(source, str) and Path(source).exists() and Path(source).is_file()

    def scan(self, source, camera_id, camera_location, display=True):
        ALERTS_DIR.mkdir(parents=True, exist_ok=True)
        LIVE_DIR.mkdir(parents=True, exist_ok=True)
        cases = self.registry.list_active()
        is_file = self._is_file_source(source)
        ...
        fpsq = deque(maxlen=FPS_SMOOTHING)
        idx = 0
        fps = 0.0
        face_count = 0
        matched_count = 0
        ...
        try:
            while True:
                ...
                if not ok:
                    if is_file:
                        # Demo/video CCTV source: loop forever instead of stopping the worker.
                        cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
                        self.tracker = SimpleFaceTracker()
                        self.cached = []
                        time.sleep(0.03)
                        continue
                    # Webcam/RTSP: transient read failures should not instantly kill the worker.
                    update_camera_status(camera_id, camera_location, source, False, fps, face_count, matched_count, 'Read failed; reconnecting')
                    cap.release()
                    time.sleep(1.0)
                    cap = cv2.VideoCapture(source)
                    if not cap.isOpened():
                        time.sleep(1.0)
                        continue
                    update_camera_status(camera_id, camera_location, source, True, fps, 0, 0, None)
                    continue
                ...
                if idx % PROCESS_EVERY_N_FRAMES == 0 and cases:
                    ...
                    for face, b, tid in zip(faces, boxes, tids):
                        ...
                        if m:
                            ...
                            if self.matcher.register(m, tid):
                                ...
                                evidence = frame.copy()
                                self.draw(evidence, anns)
                                cv2.imwrite(str(p), evidence)
                                a = {
                                    'case_id': m.case_id,
                                    'name': m.name,
                                    'similarity': m.similarity,
                                    'confidence_band': m.level,
                                    'camera_id': camera_id,
                                    'camera_location': camera_location,
                                    'timestamp': ts.isoformat(),
                                    'evidence_image': str(p),
                                    'track_id': tid,
                                }
                    ...
                    self.cached = anns
                elif idx % PROCESS_EVERY_N_FRAMES == 0 and not cases:
                    # Still detect nothing intentionally while no active search exists, but keep publishing video.
                    face_count = 0
                    matched_count = 0
                    self.cached = []
                ...
                if not cases:
                    cv2.putText(frame, 'WAITING FOR ACTIVE LOST REPORTS', (18, 64), cv2.FONT_HERSHEY_SIMPLEX, .58, (0, 200, 255), 2)

                dt = time.perf_counter() - t0
                if dt > 0:
                    fpsq.append(1 / dt)
                    fps = sum(fpsq) / len(fpsq)

                if now - last_hb > 1.0:
                    update_camera_status(camera_id, camera_location, source, True, fps, face_count, matched_count, None)
                    last_hb = now
                ...
                if now - last_live >= 0.10:
                    self.publish_live_frame(camera_id, frame)
                    last_live = now

                if display:
                    cv2.imshow(f'VariMitra - {camera_id}', frame)
                    if cv2.waitKey(1) & 0xff == ord('q'):
                        break
        except Exception as e:
            update_camera_status(camera_id, camera_location, source, False, fps, face_count, matched_count, str(e))
            raise
        finally:
            cap.release()
            if display:
